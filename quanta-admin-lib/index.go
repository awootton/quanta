...
func (f *VerifyIndexCmd) Run(ctx *proxy.Context) error {
	...
	var key string
	if field.(*shared.BasicAttribute).IsBSI() {
		key = fmt.Sprintf("%s/%s/%s", f.Table, field.(*shared.BasicAttribute).FieldName, tf)
	} else {
	...
	req := &pb.SyncStatusRequest{
		...
		ModTime:  int64(-1),
		Index:    f.Table,
		Field:    field.(*shared.BasicAttribute).FieldName,
		Time:     ts.UnixNano(),
	}
	...
	if len(indices) >= 1 {
		...
		resBsi := roaring64.NewBSI(int64(field.(*shared.BasicAttribute).MaxValue), int64(field.(*shared.BasicAttribute).MinValue))
		if len(res.Data) == 0 && res.Cardinality > 0 {
			return fmt.Errorf("deserialize sync response - BSI index out of range %d, Index = %s, Field = %s",
				len(res.Data), f.Table, field.(*shared.BasicAttribute).FieldName)
		}
